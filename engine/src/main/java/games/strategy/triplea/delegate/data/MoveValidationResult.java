...
public class MoveValidationResult implements Serializable, Comparable<MoveValidationResult> {
  ...
  /** The units disallowed with the given warning, or an empty collection. */
  public Collection<Unit> getDisallowedUnits(final String warning) {
    final int index = disallowedUnitWarnings.indexOf(warning);
    return index == -1 ? List.of() : new ArrayList<>(disallowedUnitsList.get(index));
  }
